...
function PcPlays() {
  ...
  // se ejecute al menos 1.5 sgds = 1500
  setTimeout(() => {
    let played = false;
...
function playerPlays() {
  ...
  cells.forEach((cell, i) => {
    ...
    console.log("Print column ", column, " Row ", row);
    if (board[row][column] === "") {
...
function checkIfCanWin() {
  const arr = JSON.parse(JSON.stringify(board)); // copia profunda, unidimensional [...arr]

  for (let i = 0; i < arr.length; i++) {
    for (let j = 0; j < arr.length; j++) {
      // reemplaza el tablero x objetos
      if (arr[i][j] === "X") {
  ...
  // asigno a variables cada posicion
  const p1 = arr[0][0];
  const p2 = arr[0][1];
  const p3 = arr[0][2];
  const p4 = arr[1][0];
